...
//timer_sleep을 위한 리스트, 원소로는 뭐를 넣어줄까. . . .
...
/* Transitions a blocked thread T to the ready-to-run state.
   ...
   update other data. */
bool priority_less_func(struct list_elem *a, struct list_elem *b) {
...
thread_unblock (struct thread *t) 
{
  ...
  list_push_back (&ready_list, &t->elem); //t->elem 의 주소를 push_back하는거임 + 항상 레디리스트는 priority순으로 정렬되어있어야할듯함.
...
thread_yield (void) 
{
  ...
  // msg("schedule hello");
  intr_set_level (old_level); /* 전으로 회귀*/
}
...
schedule (void) /* 현재 실행하고 있는 놈보다 더 큰 우선순위를 갖은 놈에게 cpu를 양보해야한다 */
{
  ...
  ASSERT (cur->status != THREAD_RUNNING); /* 쓰레드 러닝이면 안됨 -> 이전에 READY로 바꿔놓음*/
  ASSERT (is_thread (next)); /* idle이 아닌지 확인? */
